    ...
    let partialInterop = {
        ...
        fn() {
            ...
            for (const [key, { sources, nativeStyleToProp }] of options.config) {
                ...
                if (prop) {
                    if (Array.isArray(prop)) {
                        ...
                        for (const style of prop) {
                            if (!style)
                                continue;
                            dynamicStyles ||= globals_1.styleMetaMap.has(style);
                            stylesToFlatten.push((0, stylesheet_1.getGlobalStyle)(style));
                        }
                    }
                    else {
                        dynamicStyles ||= globals_1.styleMetaMap.has(prop);
                ...
                for (const sourceProp of sources) {
                    ...
                    for (const className of source.split(/\s+/)) {
                        ...
                        if (Array.isArray(styles)) {
                            ...
                            dynamicStyles ||= globals_1.styleMetaMap.has(styles);
                ...
                const meta = globals_1.styleMetaMap.get(style);
                style = { ...style };
                if (meta) {
                    globals_1.styleMetaMap.set(style, meta);
                }
                if (meta) {
