    ...
    register(options) {
        ...
        if (options.declarations) {
            for (const [name, styles] of Object.entries(options.declarations)) {
                ...
                if (taggedStyles) {
                    globals_1.globalStyles.set(name, taggedStyles);
                }
            }
        }
...
function tagStyles(name, styles) {
    if (Array.isArray(styles)) {
        ...
        const taggedStyles = styles.map((s) => {
            ...
            didTag ||= Boolean(s.style && globals_1.styleMetaMap.has(s.style));
            return taggedStyle;
        });
        if (didTag) {
        ...
        if (styles.animations) {
            ...
            if (names) {
                for (const name of names) {
                    if (name.type === "none")
                        continue;
                    const animationMeta = globals_1.animationMap.get(name.value);
                    if (!animationMeta)
                        continue;
        ...
        globals_1.styleSpecificity.set(styles.style, styles.specificity);
        return styles.style;
    }
}
function getGlobalStyle(style) {
    if (typeof style === "string") {
        ...
        return globals_1.globalStyles.get(style);
    }
    else {
        return globals_1.opaqueStyles.get(style) ?? style;
    }
}
exports.getGlobalStyle = getGlobalStyle;
function getSpecificity(style) {
    return style
        ? globals_1.styleSpecificity.get(style) ?? exports.inlineSpecificity
